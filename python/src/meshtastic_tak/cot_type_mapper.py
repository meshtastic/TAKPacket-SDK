...
When a CoT type string is not in the known mapping — either because it's new
(a v2.1 peer added a type the v2 receiver doesn't know) or because it's
legitimately niche — :meth:`CotTypeMapper.type_to_enum` returns
:data:`COTTYPE_OTHER` (0) and the caller populates ``cot_type_str`` (field 23)
with the full original string. On the wire, the combination
``cot_type_id = 0`` + ``cot_type_str = "…"`` is the canonical way to carry
unknown types without losing information: the reconstructed CoT XML uses
``cot_type_str`` directly, so ``<event type="…">`` comes back byte-identical
