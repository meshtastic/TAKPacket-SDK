...
def _scale_latitude_i(lat: float) -> int:
    """Scale a latitude degree value into protobuf's ``sfixed32 latitude_i = lat * 1e7``,
    clamping to the legal ±90° range first. Without the clamp, malformed XML
    (``lat="500"``) would multiply to 5e9 and wrap silently through the Int32
    cast, producing a plausible-but-wrong coordinate on the wire.
    """
    clamped = max(-90.0, min(90.0, lat))
    return int(round(clamped * 1e7))


def _scale_longitude_i(lon: float) -> int:
...
def _parse_casevac_enum(raw: str, name_map: dict, max_value: int) -> int:
...
class CotXmlParser:
    def parse(self, cot_xml: str) -> atak_pb2.TAKPacketV2:
        ...
        if point is not None:
            pkt.latitude_i = _scale_latitude_i(float(point.get("lat", "0")))
            pkt.longitude_i = _scale_longitude_i(float(point.get("lon", "0")))
        ...
        casevac_title = ""
        casevac_medline_remarks = ""
        casevac_urgent_count = 0
        casevac_urgent_surgical_count = 0
        casevac_priority_count = 0
        casevac_routine_count = 0
        casevac_convenience_count = 0
        casevac_equipment_detail = ""
        casevac_zone_protected_coord = ""
        casevac_terrain_slope_dir = ""
        casevac_terrain_other_detail = ""
        casevac_marked_by = ""
        casevac_obstacles = ""
        casevac_winds_are_from = ""
        casevac_friendlies = ""
        casevac_enemy = ""
        casevac_hlz_remarks = ""
        casevac_zmist: list = []
        ...
        def handle_link(elem):
            ...
            if point_attr is not None:
                ...
                try:
                    ...
                    return
                plati = _scale_latitude_i(plat)
                ploni = _scale_longitude_i(plon)
                ...
                if cot_type_str in ("b-t-f-d", "b-t-f-r"):
                    ...
                    cot_type_str.startswith("a-f-G-U-C")
                    or cot_type_str == "t-x-d-d"
                    or cot_type_str == "b-r-f-h-c"
                ):
                    # Skip marker detection when:
                    # - cot_type is a PLI (a-f-G-U-C / a-f-G-U-C-I) — the link is a self-ref, not a parent
                    # - cot_type is a delete event (t-x-d-d) — the link references what to delete
                    # - cot_type is casevac (b-r-f-h-c) — the link is the patient's parent ref, not a marker
                    # Don't set has_marker_data — these types have their own classification.
                    pass
                else:
                    # Marker parent link
        ...
        for elem in detail:
            ...
            if tag == "contact":
                ...
                casevac_precedence = _parse_casevac_enum(elem.get("precedence", ""), _PRECEDENCE_MAP, 5)
                eq = 0
                ...
                if elem.get("equipment_other") == "true": eq |= 0x20
                ...
                try:
                    ...
                    pass
                casevac_security = _parse_casevac_enum(elem.get("security", ""), _SECURITY_MAP, 4)
                casevac_hlz_marking = _parse_casevac_enum(elem.get("hlz_marking", ""), _HLZ_MARKING_MAP, 5)
                ...
                nationality_keys = {
                    ...
                    "nonus_military": "non_us_military",
                    "non_us_civilian": "non_us_civilian",
                    "nonus_civilian": "non_us_civilian",
                    "epw": "epw",
                    "child": "child",
                }
                for attr_name, canonical in nationality_keys.items():
                    ...
                    try:
                        ...
                        continue
                    if canonical == "us_military": casevac_us_military = val
                    elif canonical == "us_civilian": casevac_us_civilian = val
                    elif canonical == "non_us_military": casevac_non_us_military = val
                    elif canonical == "non_us_civilian": casevac_non_us_civilian = val
                    elif canonical == "epw": casevac_epw = val
                    elif canonical == "child": casevac_child = val
                tf = 0
                ...
                # --- v2.x medline extensions ---------------------------------
                casevac_title = elem.get("title", "")
                casevac_medline_remarks = elem.get("medline_remarks", "")
                for count_attr in ("urgent", "urgent_surgical", "priority", "routine", "convenience"):
                    raw = elem.get(count_attr)
                    if raw is None:
                        continue
                    try:
                        val = int(raw)
                        ...
                        continue
                    if count_attr == "urgent": casevac_urgent_count = val
                    elif count_attr == "urgent_surgical": casevac_urgent_surgical_count = val
                    elif count_attr == "priority": casevac_priority_count = val
                    elif count_attr == "routine": casevac_routine_count = val
                    elif count_attr == "convenience": casevac_convenience_count = val
                casevac_equipment_detail = elem.get("equipment_detail", "")
                casevac_zone_protected_coord = elem.get("zone_protected_coord", "")
                casevac_terrain_slope_dir = elem.get("terrain_slope_dir", "")
                casevac_terrain_other_detail = elem.get("terrain_other_detail", "")
                casevac_marked_by = elem.get("marked_by", "")
                casevac_obstacles = elem.get("obstacles", "")
                casevac_winds_are_from = elem.get("winds_are_from", "")
                casevac_friendlies = elem.get("friendlies", "")
                casevac_enemy = elem.get("enemy", "")
                casevac_hlz_remarks = elem.get("hlz_remarks", "")
                # <zMistsMap> children — each <zMist> is one patient record
                zmists_map = elem.find("zMistsMap")
                if zmists_map is not None:
                    for z_elem in zmists_map.findall("zMist"):
                        casevac_zmist.append({
                            "title": z_elem.get("title", ""),
                            "z": z_elem.get("z", ""),
                            "m": z_elem.get("m", ""),
                            "i": z_elem.get("i", ""),
                            "s": z_elem.get("s", ""),
        ...
        if not has_emergency_data and cot_type_str.startswith("b-a-"):
            has_emergency_data = True
            if emergency_type_int == 0:
                emergency_type_int = _EMERGENCY_TYPE_BY_COT.get(cot_type_str, 0)
        if not has_casevac_data and cot_type_str == "b-r-f-h-c":
            has_casevac_data = True

        # Delete events (t-x-d-d) short-circuit everything — their <link>
        # references what to delete, not a marker parent.
        is_delete_event = cot_type_str == "t-x-d-d"
        ...
        if is_delete_event:
            ...
            # v2.x medline extensions
            c.title = casevac_title
            c.medline_remarks = casevac_medline_remarks
            c.urgent_count = casevac_urgent_count
            c.urgent_surgical_count = casevac_urgent_surgical_count
            c.priority_count = casevac_priority_count
            c.routine_count = casevac_routine_count
            c.convenience_count = casevac_convenience_count
            c.equipment_detail = casevac_equipment_detail
            c.zone_protected_coord = casevac_zone_protected_coord
            c.terrain_slope_dir = casevac_terrain_slope_dir
            c.terrain_other_detail = casevac_terrain_other_detail
            c.marked_by = casevac_marked_by
            c.obstacles = casevac_obstacles
            c.winds_are_from = casevac_winds_are_from
            c.friendlies = casevac_friendlies
            c.enemy = casevac_enemy
            c.hlz_remarks = casevac_hlz_remarks
            for z in casevac_zmist:
                z_msg = c.zmist.add()
                z_msg.title = z["title"]
                z_msg.z = z["z"]
                z_msg.m = z["m"]
    ...
    def extract_raw_detail_bytes(cot_xml: str) -> bytes:
        ...
        Useful for callers that want to build a ``raw_detail`` fallback
        packet alongside the typed packet.
