...
public class CotXmlParser
{
    ...
    /// malformed XML (<c>lat="500"</c>) would multiply to 5e9 and wrap silently
    /// through the Int32 cast, producing a plausible-but-wrong coordinate.
    /// </summary>
    private static int ScaleLatitudeI(double lat)
    {
        var clamped = Math.Max(-90.0, Math.Min(90.0, lat));
        return (int)Math.Round(clamped * 1e7);
    }

    /// <summary>
    /// Scale a longitude degree value into <c>sfixed32 longitude_i</c>, clamped to ±180°.
    /// </summary>
    private static int ScaleLongitudeI(double lon)
    {
        var clamped = Math.Max(-180.0, Math.Min(180.0, lon));
        return (int)Math.Round(clamped * 1e7);
    }
    ...
    /// </summary>
    private static TEnum ParseCasevacEnum<TEnum>(
        string? raw,
        Dictionary<string, TEnum> nameMap) where TEnum : struct, Enum
    {
    ...
    public TAKPacketV2 Parse(string cotXml)
    {
        ...
        string casevacTitle = "", casevacMedlineRemarks = "";
        uint casevacUrgentCount = 0, casevacUrgentSurgicalCount = 0;
        uint casevacPriorityCount = 0, casevacRoutineCount = 0, casevacConvenienceCount = 0;
        string casevacEquipmentDetail = "", casevacZoneProtectedCoord = "";
        string casevacTerrainSlopeDir = "", casevacTerrainOtherDetail = "";
        string casevacMarkedBy = "";
        string casevacObstacles = "", casevacWindsAreFrom = "";
        string casevacFriendlies = "", casevacEnemy = "", casevacHlzRemarks = "";
        var casevacZmist = new List<ZMistEntry>();
        ...
        void HandleLink(XElement el)
        {
            ...
            else if (linkUid != null && relation == "p-p" && linkType.Length > 0)
            {
                ...
                else if (typeStr.StartsWith("a-f-G-U-C")
                         || typeStr == "t-x-d-d"
                         || typeStr == "b-r-f-h-c")
                {
        ...
        foreach (var el in detail.Elements())
        {
            switch (el.Name.LocalName)
            {
                ...
                case "_medevac_":
                    hasCasevacData = true;
                    casevacPrecedence = ParseCasevacEnum(el.Attribute("precedence")?.Value, PrecedenceMap);
                    ...
                    casevacTerrainSlopeDir = el.Attribute("terrain_slope_dir")?.Value ?? "";
                    casevacTerrainOtherDetail = el.Attribute("terrain_other_detail")?.Value ?? "";
                    casevacMarkedBy = el.Attribute("marked_by")?.Value ?? "";
                    casevacObstacles = el.Attribute("obstacles")?.Value ?? "";
                    casevacWindsAreFrom = el.Attribute("winds_are_from")?.Value ?? "";
                    casevacFriendlies = el.Attribute("friendlies")?.Value ?? "";
                    casevacEnemy = el.Attribute("enemy")?.Value ?? "";
                    casevacHlzRemarks = el.Attribute("hlz_remarks")?.Value ?? "";
                    // <zMistsMap> children — each <zMist> is one patient record
                    var zmistsMapEl = el.Element("zMistsMap");
                    if (zmistsMapEl is not null)
                    {
                        foreach (var zEl in zmistsMapEl.Elements("zMist"))
                        {
                            casevacZmist.Add(new ZMistEntry
                            {
                                Title = zEl.Attribute("title")?.Value ?? "",
                                Z = zEl.Attribute("z")?.Value ?? "",
                                M = zEl.Attribute("m")?.Value ?? "",
                                I = zEl.Attribute("i")?.Value ?? "",
                                S = zEl.Attribute("s")?.Value ?? "",
                                T = zEl.Attribute("t")?.Value ?? "",
                            });
                        }
                    }
                    break;
        ...
        {
            hasEmergencyData = true;
            if (emergencyTypeEnum == EmergencyAlert.Types.Type.Unspecified)
                emergencyTypeEnum = EmergencyTypeFromCotType(typeStr);
        }
        ...
        bool isDeleteEvent = typeStr == "t-x-d-d";
        ...
        else if (hasCasevacData)
        {
            ...
            casevac.Zmist.AddRange(casevacZmist);
            pkt.Casevac = casevac;
        }
    ...
    /// normalization, no re-escaping.  Useful for callers that want to
    /// build a <c>raw_detail</c> fallback packet alongside the typed
    /// packet.
    /// </summary>
    /// <remarks>
