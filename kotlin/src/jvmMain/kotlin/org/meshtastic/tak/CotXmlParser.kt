...
class CotXmlParser {
    ...
    companion object {
        ...
        // Aircraft-remarks scrapers, compiled once. ATAK emits these as a
        // free-text summary inside <remarks> when the structured _aircot_
        // detail is missing; we pull ICAO/reg/flight/type/squawk/category
        // back out so the receiver can populate AircraftTrack.
         ...
         * malformed XML (`lat="500"`) would multiply to 5e9 and wrap silently through
         * Int32, producing a plausible-but-wrong coordinate on the wire.
         */
         ...
         * ("1", "2", "3"). Newer ATAK / iTAK emits the numeric form; older
            ...
            when (eventType) {
                XmlPullParser.START_TAG -> {
                    when (parser.name) {
                        ...
                        "_medevac_" -> {
                            hasCasevacData = true
                            casevacPrecedence = parseCasevacEnum(parser.getAttributeValue(null, "precedence"), precedenceMap, 5)
                            ...
                            if (parser.getAttributeValue(null, "equipment_other") == "true") eq = eq or 0x20
                            ...
                            casevacSecurity = parseCasevacEnum(parser.getAttributeValue(null, "security"), securityMap, 4)
                            casevacHlzMarking = parseCasevacEnum(parser.getAttributeValue(null, "hlz_marking"), hlzMarkingMap, 5)
                            ...
                            casevacNonUsMilitary = (parser.getAttributeValue(null, "non_us_military")
                                ?: parser.getAttributeValue(null, "nonus_military"))?.toIntOrNull() ?: 0
                            casevacNonUsCivilian = (parser.getAttributeValue(null, "non_us_civilian")
                                ?: parser.getAttributeValue(null, "nonus_civilian"))?.toIntOrNull() ?: 0
                            ...
                            // --- v2.x medline extensions ----------------------
                            casevacTitle = parser.getAttributeValue(null, "title") ?: ""
                            casevacMedlineRemarks = parser.getAttributeValue(null, "medline_remarks") ?: ""
                            // Patient counts by precedence level (newer ATAK format)
                            casevacUrgentCount = parser.getAttributeValue(null, "urgent")?.toIntOrNull() ?: 0
                            casevacUrgentSurgicalCount = parser.getAttributeValue(null, "urgent_surgical")?.toIntOrNull() ?: 0
                            casevacPriorityCount = parser.getAttributeValue(null, "priority")?.toIntOrNull() ?: 0
                            casevacRoutineCount = parser.getAttributeValue(null, "routine")?.toIntOrNull() ?: 0
                            casevacConvenienceCount = parser.getAttributeValue(null, "convenience")?.toIntOrNull() ?: 0
                            casevacEquipmentDetail = parser.getAttributeValue(null, "equipment_detail") ?: ""
                            casevacZoneProtectedCoord = parser.getAttributeValue(null, "zone_protected_coord") ?: ""
                            casevacTerrainSlopeDir = parser.getAttributeValue(null, "terrain_slope_dir") ?: ""
                            casevacTerrainOtherDetail = parser.getAttributeValue(null, "terrain_other_detail") ?: ""
                            casevacMarkedBy = parser.getAttributeValue(null, "marked_by") ?: ""
                            casevacObstacles = parser.getAttributeValue(null, "obstacles") ?: ""
                            casevacWindsAreFrom = parser.getAttributeValue(null, "winds_are_from") ?: ""
                            casevacFriendlies = parser.getAttributeValue(null, "friendlies") ?: ""
                            casevacEnemy = parser.getAttributeValue(null, "enemy") ?: ""
                            casevacHlzRemarks = parser.getAttributeValue(null, "hlz_remarks") ?: ""
                        }
                        // <zMist> inside <zMistsMap> inside <_medevac_> —
                        // one entry per patient. We capture the attributes at
                        // START_TAG and append to the list immediately since
                        // ATAK always self-closes these.
                        "zMist" -> {
                            if (hasCasevacData) {
                                casevacZmistEntries.add(
                                    TakPacketV2Data.Payload.CasevacReport.ZMistEntry(
                                        title = parser.getAttributeValue(null, "title") ?: "",
                                        z = parser.getAttributeValue(null, "z") ?: "",
                                        m = parser.getAttributeValue(null, "m") ?: "",
                                        i = parser.getAttributeValue(null, "i") ?: "",
                                        s = parser.getAttributeValue(null, "s") ?: "",
                                        t = parser.getAttributeValue(null, "t") ?: "",
                                    )
                                )
                            }
                        }
     ...
     * re-escaping).  Callers can use this to build a `raw_detail` fallback
     * packet for CoT types the parser can't structurally decompose.
     *
