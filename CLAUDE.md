...
- **The zstd codec is pure Kotlin in `commonMain` on EVERY target (v0.6.0).** There is ONE codec (`internal.zstd` `PureZstdEncoder` / `PureZstdDecoder`) used by jvm, all 9 native targets, js, wasmJs, and wasmWasi — no `zstd-jni`, no native `libzstd` cinterop, no `@bokuweb/zstd-wasm`. `ZstdCodec` / `DictionaryLoader` are plain `commonMain` objects (no more `expect`/`actual`); native targets need no `libzstd` at all. This is interoperable **in both directions**: our decoder reads real libzstd frames (proven by decoding every golden) and our encoder's frames are read by real libzstd (proven by the zstd-jni jvmTest oracle + the libzstd-based Swift/Python/TS/C# bindings). The `.bin` goldens are therefore **pure-Kotlin-generated** as of 0.6.0; the swap regenerated `.bin` but left `.pb` byte-identical. `zstd-jni` survives only as a jvmTest cross-check oracle.
