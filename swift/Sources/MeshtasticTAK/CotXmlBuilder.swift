...
public class CotXmlBuilder {
    ...
    private func emitCasevac(_ c: CasevacReport) -> String {
        var s = "    <_medevac_"
        if !c.title.isEmpty { s += " title=\"\(esc(c.title))\"" }
        if !c.medlineRemarks.isEmpty { s += " medline_remarks=\"\(esc(c.medlineRemarks))\"" }
        ...
        if c.urgentCount > 0 { s += " urgent=\"\(c.urgentCount)\"" }
        if c.urgentSurgicalCount > 0 { s += " urgent_surgical=\"\(c.urgentSurgicalCount)\"" }
        if c.priorityCount > 0 { s += " priority=\"\(c.priorityCount)\"" }
        if c.routineCount > 0 { s += " routine=\"\(c.routineCount)\"" }
        if c.convenienceCount > 0 { s += " convenience=\"\(c.convenienceCount)\"" }
        ...
        if c.equipmentFlags & 0x20 != 0 { s += " equipment_other=\"true\"" }
        if !c.equipmentDetail.isEmpty { s += " equipment_detail=\"\(esc(c.equipmentDetail))\"" }
        ...
        if !c.terrainSlopeDir.isEmpty { s += " terrain_slope_dir=\"\(esc(c.terrainSlopeDir))\"" }
        ...
        if !c.terrainOtherDetail.isEmpty { s += " terrain_other_detail=\"\(esc(c.terrainOtherDetail))\"" }
        if !c.zoneProtectedCoord.isEmpty { s += " zone_protected_coord=\"\(esc(c.zoneProtectedCoord))\"" }
        if !c.zoneMarker.isEmpty { s += " zone_prot_marker=\"\(esc(c.zoneMarker))\"" }
        if !c.markedBy.isEmpty { s += " marked_by=\"\(esc(c.markedBy))\"" }
        // Tier-2 situational awareness free-text
        if !c.obstacles.isEmpty { s += " obstacles=\"\(esc(c.obstacles))\"" }
        if !c.windsAreFrom.isEmpty { s += " winds_are_from=\"\(esc(c.windsAreFrom))\"" }
        if !c.friendlies.isEmpty { s += " friendlies=\"\(esc(c.friendlies))\"" }
        if !c.enemy.isEmpty { s += " enemy=\"\(esc(c.enemy))\"" }
        if !c.hlzRemarks.isEmpty { s += " hlz_remarks=\"\(esc(c.hlzRemarks))\"" }
        if c.zmist.isEmpty {
            s += "/>\n"
        } else {
            s += ">\n"
            ...
            for z in c.zmist {
                var zs = "        <zMist"
                if !z.title.isEmpty { zs += " title=\"\(esc(z.title))\"" }
                if !z.z.isEmpty { zs += " z=\"\(esc(z.z))\"" }
                if !z.m.isEmpty { zs += " m=\"\(esc(z.m))\"" }
                if !z.i.isEmpty { zs += " i=\"\(esc(z.i))\"" }
                if !z.s.isEmpty { zs += " s=\"\(esc(z.s))\"" }
                if !z.t.isEmpty { zs += " t=\"\(esc(z.t))\"" }
                zs += "/>\n"
                s += zs
            }
            s += "      </zMistsMap>\n"
